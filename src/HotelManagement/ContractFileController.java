...
public class ContractFileController implements FileController {
	...
	public static void checkIn(Hotel hotel) throws IOException {
		...
		switch (check) {
		case "y": {
			File contractFile = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "Reservation"
					+ File.separator + customer.getFirstname() + " " + customer.getLastname() + ".txt");
			...
			contractFile.renameTo(new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "In"
					+ File.separator + contractFile.getName()));
		...
		case "n": {
			...
			if (roomList.size() > 0) {
				...
				case "y": {
					...
					String path = dir + File.separator + customer.getFirstname() + " " + customer.getLastname()
							+ ".txt";
					Contract contract = new Contract(start, end, customer, room, path, price);
					contract.writeToFile();
	...
	public static void checkOut(Hotel hotel) throws IOException {
		...
		if (count > 0) {
			...
			File filee = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "Out"
					+ File.separator + file.getName());
			int frequency = 1;
			while (filee.exists()) {
				filee = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "Out"
						+ File.separator + frequency + file.getName());
				frequency++;
			}
			file.renameTo(filee);
			Room room = contract.getRoom();
	...
	public static ArrayList<Integer> showAvailableRoom(String type, Hotel hotel, String start, String end) {
		...
		while (iter.hasNext()) {
			...
			if ((end.compareTo(contractStartDay) > 0 && end.compareTo(contractEndDay) <= 0)
					|| (start.compareTo(contractStartDay) >= 0 && start.compareTo(contractEndDay) < 0)
					|| (start.compareTo(contractStartDay) <= 0 && end.compareTo(contractEndDay) >= 0)) {
	...
	public static void deleteContract(Hotel hotel) throws FileNotFoundException {
		...
		File contractLocation = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "Reservation"
				+ File.separator + contractName + ".txt");
	...
	// NEW CODE
	public static void deleteContractIn(Hotel hotel, Customer c) throws IOException {
		System.out.println("Which contract are you going to delete? (Can only delete your own)");
		File contractFile = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "In");
		File[] fs = contractFile.listFiles();
		for (File f : fs) {
			System.out.println(f.getName().substring(0, f.getName().length() - 4));
		}
		Scanner sc = new Scanner(System.in);
		String contractName = sc.nextLine();
		if (contractName.matches(c.getFirstname() + " " + c.getLastname())) {
			File contractLocation = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "In"
					+ File.separator + contractName + ".txt");
			Contract contract = readContract(hotel, contractLocation);
			hotel.getInContracts().remove(contract.getCustomer());
			File file = new File(contract.getPath());
			HotelFileController.addToDeleteFilesPermantly(file, hotel.getFile());
			System.out.println("Success! The contract has been terminated!");
			c.setStaying(false);
			c.writeToFile();
		} else
			System.out.println("Invalid input or wrong customer");
	}

	// NEW CODE
	public static void deleteContractRes(Hotel hotel, Customer c) throws IOException {
		System.out.println("Which contract are you going to delete? (Can only delete your own)");
		File contractFile = new File(hotel.getPath() + File.separator + "Contracts" + File.separator + "Reservation");
		File[] fs = contractFile.listFiles();
		for (File f : fs) {
		...
		String contractName = sc.nextLine();
		if (contractName.matches(c.getFirstname() + " " + c.getLastname())) {
			File contractLocation = new File(hotel.getPath() + File.separator + "Contracts" + File.separator
					+ "Reservation" + File.separator + contractName + ".txt");
			Contract contract = readContract(hotel, contractLocation);
			hotel.getReservationContracts().remove(contract.getCustomer());
			File file = new File(contract.getPath());
			HotelFileController.addToDeleteFilesPermantly(file, hotel.getFile());
	...
	public static void contractReport(Hotel hotel) {
		...
		case "m": {
			System.out.println(
					"Please indicate which year and which month you want to look up the report.(e.g. 2019-11)");
			String when = sc.nextLine();
			ArrayList<Contract> outContracts = hotel.getOutContracts();
			double price = 0.0;
			for (Contract c : outContracts) {

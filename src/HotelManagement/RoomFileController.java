...
public class RoomFileController implements FileController {
	...
	public static void EditRoom(File room) throws IOException {
		...
		while (toggle) {
			...
			case "Delete": {
				File roomsFile = new File(room.getParent());
				File hotelFile = new File(roomsFile.getParent());
				HotelFileController.deleteRoom2(new Room(room), new Hotel(hotelFile));
				curRoom.setEmpty(false);
				break;
	...
	public static void showRoomPic(Hotel hotel) {
		...
		try {
			switch (type) {
			case "single": {
				File sourceimage = new File("src" + File.separator + "Pictures" + File.separator + style
						+ File.separator + "SingleRoom.jpg");
			...
			case "king": {
				File sourceimage = new File(
						"src" + File.separator + "Pictures" + File.separator + style + File.separator + "KingRoom.jpg");
				image = ImageIO.read(sourceimage);
				break;
			}
			}
